...
const generateExperiences = ({navigation}) => {
  return Experiences.map((experience, index) => {
    ...
    return (
        <TouchableOpacity onPress={() => navigation.navigate("Experience Cancelation", { experienceIndex: index })}>
...
const Upcoming = ({navigation}) => {
  ...
  return(
    <View style={{ alignItems: "center", flexDirection: 'column', display: 'flex', width: "100%"}}>
      <ScrollView style={{width:"100%", height: "100%", paddingHorizontal: 30}} contentContainerStyle={{ alignItems: 'center', flex: 1, flexGrow: 1, minHeight: '100%' }}>
